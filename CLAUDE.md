...
Optimization is a mandatory follow-up pass. It must be completed as part of the task, but it must remain separate from the initial feature/fix implementation so correctness is established before optimization changes are introduced.

Complete the feature/fix correctly first, then perform the optimization pass.

Do not mix speculative optimization into the initial feature/fix diff.

Measure the relevant bottleneck before choosing the optimization; do not optimize blindly.

Complete the identified optimization work before declaring the task done.

Run the relevant regression/performance tests after optimization and confirm that behavior and correctness are preserved.

Never trade correctness, financial integrity, security, or reliability for performance.
...
- [ ] The required optimization pass was completed separately from the feature/fix diff, measured appropriately, and verified with the relevant tests.
