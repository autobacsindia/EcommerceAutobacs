...
export default function GlobalError({
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-obsidian-deep">
          ...
          {process.env.NODE_ENV === 'development' && (
            <details className="mt-6 text-left">
              ...
              <div className="mt-2 p-4 bg-obsidian-raised rounded text-xs overflow-auto max-h-64">
                ...
                <pre className="text-ink/80 whitespace-pre-wrap">
                  {error.stack}
                </pre>
                {error.digest && (
                  <p className="mt-2 text-ink-muted">Digest: {error.digest}</p>
                )}
              </div>
            </details>
          )}
