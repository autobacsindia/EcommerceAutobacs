...
interface VehicleProductsResponse {
  success: boolean;
  vehicle: Vehicle;
  products: Product[];
  pagination: {
...
export default function VehicleProductsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const fetchVehicleProducts = async () => {
    try {
      ...
      console.error('Failed to fetch vehicle products:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const searchAvailableProducts = async () => {
    ...
    try {
      ...
      // Filter out products already mapped to this vehicle
      const mappedProductIds = new Set(mappedProducts.map(p => p._id));
      const unmappedProducts = (response.products || []).filter(
        p => !mappedProductIds.has(p._id)
      );
  ...
  const handleMapProducts = async () => {
    ...
    try {
      await apiClient.post(API_ENDPOINTS.VEHICLE_MAP_PRODUCTS(id), {
  ...
  const handleUnmapProduct = async (productId: string, productName: string) => {
    if (!confirm(`Are you sure you want to remove "${productName}" from ${vehicle?.name}?`)) return;

    try {
      await apiClient.delete(API_ENDPOINTS.VEHICLE_UNMAP_PRODUCT(id, productId));
      fetchVehicleProducts();
    } catch (err: any) {
    ...
    <div className="p-8">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold">{vehicle?.name} Products</h1>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        {mappedProducts.length === 0 && (
          <div className="text-center py-12 text-gray-500">
            No products mapped to this vehicle yet.{' '}
            <button
              onClick={() => setShowAddModal(true)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col">
            ...
            <div className="p-4 border-b flex items-center justify-between">
              <h2 className="text-xl font-bold">Add Products to {vehicle?.name}</h2>
              <button
                onClick={() => {
                  setShowAddModal(false);
                  setSearchTerm('');
