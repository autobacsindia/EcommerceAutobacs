...
// ─── Article Form ──────────────────────────────────────────────────────────────
const EMPTY_ARTICLE = {
  title: '', type: 'news' as 'news' | 'blog', coverImage: '', excerpt: '',
...
export default function AdminMediaPage() {
  ...
  const fetchMediaItems = useCallback(async () => {
    ...
    try {
      ...
      if (tab === 'gallery') params.type = 'image';
      if (tab === 'videos') params.type = 'video';
  ...
  async function saveArticle(e: React.FormEvent) {
    ...
    try {
      ...
      if (editingArticle) {
        await apiClient.put(API_ENDPOINTS.ADMIN_MEDIA_ARTICLE(editingArticle._id), payload);
      } else {
    ...
    apiClient.get<any>(API_ENDPOINTS.ADMIN_MEDIA_ARTICLE(article._id))
      .then(res => {
        if (res.success) {
          const a = res.data;
          setArticleForm({
            title: a.title, type: a.type, coverImage: a.coverImage,
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Media &amp; News</h1>
      ...
      {showStats && stats && (
        <div className="mb-6 bg-gray-50 rounded-xl border border-gray-200 p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 mb-5">
            {[
              { label: 'Total Articles', value: stats.articles.total, color: 'text-gray-900' },
              { label: 'Published', value: stats.articles.published, color: 'text-green-600' },
              { label: 'News', value: stats.articles.news, color: 'text-blue-600' },
              { label: 'Blogs', value: stats.articles.blogs, color: 'text-green-600' },
          ...
          {stats.topArticles.length > 0 && (
            <div>
              <p className="text-sm font-semibold text-gray-700 mb-2">🔥 Most Viewed Articles</p>
              <div className="space-y-1.5">
                {stats.topArticles.map((a, i) => (
                  <div key={a._id} className="flex items-center gap-3 bg-white rounded-lg px-3 py-2 border border-gray-100">
                    ...
                    <Link href={`/media/${a.type}/${a.slug}`} target="_blank" className="flex-1 text-sm text-gray-700 hover:text-red-600 line-clamp-1 transition-colors">{a.title}</Link>
                    <span className="text-xs font-medium text-gray-500 flex items-center gap-1"><Eye className="h-3 w-3" />{a.views}</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${a.type === 'news' ? 'bg-blue-100 text-blue-600' : 'bg-green-100 text-green-600'}`}>{a.type}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredArticles.length === 0 ? (
                    <tr><td colSpan={7} className="text-center py-12 text-gray-400">No articles found</td></tr>
                    ...
                    <tr key={article._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${article.type === 'news' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}>
                          {article.type}
                        </span>
                      </td>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl my-8">
            ...
            <form onSubmit={saveArticle} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setArticleForm(f => ({ ...f, type: e.target.value as 'news' | 'blog' }))}
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                <button type="button" onClick={() => { setShowArticleForm(false); setEditingArticle(null); }} className="px-4 py-2 text-sm border border-gray-200 rounded-lg hover:bg-gray-50">
                  Cancel
                </button>
                <button type="submit" disabled={articleSaving} className="px-5 py-2 text-sm bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-60">
