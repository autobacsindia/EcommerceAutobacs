...
export function useSnapPager({
    ...
    const io = new IntersectionObserver(
      (entries) => {
        for (const entry of entries) {
          ...
          if (index >= 0 && index !== activeRef.current) onChangeRef.current(index);
        }
      },
  ...
  //
  // Naturally loop-free: when the change originated from the user's own scroll,
  // the strip is already at that offset and we bail before touching it. No
  // "programmatic scroll in progress" flag needed.
