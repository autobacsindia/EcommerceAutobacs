...
export default function CheckoutSummary({ items, isAuthenticated, shippingCost = 0, onChange }: Props) {
  ...
  return (
    <div className="space-y-5">
      {/* ── Coupon ──────────────────────────────────────────────────────────── */}
      <div>
        <h3 className="text-xs font-condensed font-bold text-[#555555] uppercase tracking-widest mb-2 flex items-center gap-2">
        ...
        {quote?.appliedCoupon ? (
          <div className="flex items-center justify-between bg-green-500/10 border border-green-500/30 rounded-sm px-3 py-2">
            <span className="text-green-400 font-condensed font-bold text-sm uppercase tracking-wide">
              {quote.appliedCoupon.code} applied
            </span>
            <button onClick={removeCoupon} className="text-[#555555] hover:text-red-400 transition-colors" title="Remove coupon">
              <X className="h-4 w-4" />
            </button>
          </div>
        ) : (
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-[#161616] border border-[#252525] text-white placeholder:text-[#555555] rounded-sm px-3 py-2 text-sm font-body uppercase tracking-wide focus:outline-none focus:border-[#3B9EE8] transition-colors"
            />
            <button
              onClick={applyCoupon}
              disabled={!couponInput.trim()}
              className="px-4 bg-[#3B9EE8] hover:bg-[#1A6FB5] text-white font-condensed font-bold uppercase tracking-widest text-sm rounded-sm disabled:bg-[#252525] disabled:text-[#555555] disabled:cursor-not-allowed transition-colors"
            >
              Apply
            </button>
          </div>
        ...
        {appliedCode && quote?.couponError && (
          <p className="text-red-400 text-xs font-body mt-1.5">{quote.couponError}</p>
        )}
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {available.map((c) => (
              <button
                ...
                className="text-xs font-condensed font-bold uppercase tracking-wide text-[#3B9EE8] border border-[#3B9EE8]/30 hover:border-[#3B9EE8] rounded-sm px-2 py-1 transition-colors"
                title={c.description || ''}
              >
                {c.code}
              </button>
      ...
      {karmaEnabled && (
        <div className="border-t border-[#252525] pt-4">
          ...
          <p className="text-[#C4C4C4] font-body text-sm mb-2">
            Balance: <span className="text-white font-condensed font-bold">{karma!.balance}</span> pts
            <span className="text-[#555555]"> · 1 pt = {money(karma!.config.pointValueInRupees)}</span>
          </p>
          {maxRedeem > 0 ? (
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-28 bg-[#161616] border border-[#252525] text-white rounded-sm px-3 py-2 text-sm font-body focus:outline-none focus:border-[#3B9EE8] transition-colors"
              />
              <button
                onClick={() => setRedeemPoints(maxRedeem)}
                className="px-3 py-2 text-xs font-condensed font-bold uppercase tracking-widest text-[#3B9EE8] border border-[#3B9EE8]/30 hover:border-[#3B9EE8] rounded-sm transition-colors"
              >
                Use max ({maxRedeem})
              </button>
            </div>
          ) : (
            <p className="text-[#555555] font-body text-xs">
      ...
      {/* ── Breakdown ───────────────────────────────────────────────────────── */}
      <div className="border-t border-[#252525] pt-4">
        <h3 className="text-xs font-condensed font-bold text-[#555555] uppercase tracking-widest mb-3 flex items-center gap-2">
          Order Summary {loading && <Loader2 className="h-3 w-3 animate-spin text-[#3B9EE8]" />}
        </h3>
        ...
        <div className="flex justify-between border-t border-[#252525] pt-3 mt-3">
          ...
          <span className="text-xl font-condensed font-bold text-[#3B9EE8]">{money(quote?.totalAmount ?? 0)}</span>
        </div>
      </div>
    </div>
  );
...
function Row({ label, value, positive, muted }: { label: string; value: string; positive?: boolean; muted?: boolean }) {
  return (
    <div className="flex justify-between text-sm">
      <span className="text-[#555555] font-body">{label}</span>
      <span className={`font-body ${positive ? 'text-green-400' : muted ? 'text-[#555555]' : 'text-[#C4C4C4]'}`}>{value}</span>
    </div>
  );
}
