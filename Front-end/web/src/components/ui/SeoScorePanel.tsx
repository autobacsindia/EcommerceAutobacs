...
export default function SeoScorePanel({ data }: { data: SeoData }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4 space-y-4">
      <h2 className="text-sm font-semibold text-gray-800 uppercase tracking-wide">SEO Score</h2>
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-7 pr-3 py-1.5 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-800 bg-white placeholder:text-gray-400"
          />
        </div>
      ...
      <div className="flex flex-col items-center py-1">
        ...
        <span className="text-[10px] text-gray-400 mt-0.5">Updates live as you fill the form</span>
      </div>
      ...
      <div className="space-y-2">
        {groups.map(group => {
          ...
          return (
            <details
              key={group.title}
              className={`border rounded-md overflow-hidden ${group.isKeywordGroup ? 'border-blue-200' : 'border-gray-100'}`}
              open
            >
              <summary className={`flex items-center justify-between px-3 py-2 text-xs font-semibold cursor-pointer select-none list-none
                ${group.isKeywordGroup
                  ? allPassed ? 'bg-blue-50 text-blue-700' : 'bg-blue-50 text-blue-700'
                  : allPassed ? 'bg-green-50 text-green-700' : 'bg-gray-50 text-gray-700'
                }`}
              >
                  ...
                  return (
                    <div key={check.label} className="px-3 py-2 flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {check.tip && (
                          <p className="text-[10px] text-gray-400 mt-0.5 leading-snug">{check.tip}</p>
                        )}
                      </div>
