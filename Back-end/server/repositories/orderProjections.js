 ...
 * production orders: the average order serialises to 1829 B, of which the customer list
 * reads 440 B and the admin list 541 B — so 76% / 70% of every list response was fields
 * no screen renders. Per page that is 17.9 KB → 4.3 KB (customer, limit 10) and
 * 35.7 KB → 10.6 KB (admin, limit 20).
 *
 ...
 *
 * ⚠️ Adding a field to one of these screens means adding it here too.
 */

/** Customer /orders list — see Front-end/web/src/app/orders/page.tsx. */
export const CUSTOMER_LIST_FIELDS =
  'orderNumber status totalAmount createdAt trackingNumber items shipments';
...
export const ADMIN_LIST_FIELDS =
  'orderNumber createdAt status paymentStatus cancelledBy totalAmount refundDetails user items shipments';
