...
// Import configuration with adjusted timeouts
const CONFIG = {
  batchSize: parseInt(process.env.IMPORT_BATCH_SIZE) || 10, // Smaller batch size
  delayBetweenBatches: parseInt(process.env.IMPORT_DELAY_BETWEEN_BATCHES) || 2000, // Longer delay
  retryLimit: 3,
  retryDelay: 1000
};
...
// Connect to MongoDB with adjusted timeouts
async function connectToDatabase() {
  try {
    console.log('🔗 Connecting to MongoDB with adjusted timeouts...');
...
async function modifiedImport() {
  let metadata = loadMetadata();
  const startTime = new Date();
  const importId = `import-${startTime.getTime()}`;
  
  console.log('🚀 Starting modified product import...');
