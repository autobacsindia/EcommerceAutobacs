// Improved Profender product import with better category selection
...
const CATEGORY_PRIORITY = [
  // Specific product type categories
  ...
  'Brake System', 'Electronics', 'Audio', 'Protection Kit',
  
  // General categories (lower priority)
  'Body Parts', 'Exterior Accessories', 'Accessories', 'Parts',
...
async function importProfenderWithBetterCategories() {
  try {
    ...
    console.log('🚀 Starting Profender product import with improved category selection...');
    ...
    for (let i = 0; i < wpProducts.length; i++) {
      ...
      try {
        ...
        if (wpProduct.categories && wpProduct.categories.length > 0) {
          ...
          if (!category) {
            // Create new category
            console.log(`   ➕ Creating new category: ${bestCategory.name}`);
            category = new Category({
              name: bestCategory.name,
              slug: bestCategory.slug,
              description: bestCategory.description || `Category for ${bestCategory.name}`
            });
            await category.save();
        ...
        const productData = {
          ...
          brand: 'Profender',
          category: categoryId, // Use the best category
...
// Run import
importProfenderWithBetterCategories();