// Import relevant Profender products matching specific categories
...
async function importRelevantProfenderProducts() {
  try {
    ...
    const relevantProducts = wpProducts.filter(product => {
      ...
      // Check if product matches any of our keywords in name, description or categories
    ...
    console.log(`🎯 Found ${relevantProducts.length} relevant Profender products to import:`);
    
    relevantProducts.forEach((product, index) => {
      console.log(`${index + 1}. ${product.name}`);
    });
    
    // Import each relevant product
    let importedCount = 0;
    let updatedCount = 0;
    let failedCount = 0;
    
    for (const wpProduct of relevantProducts) {
    ...
    console.error('💥 Error importing relevant Profender products:', error.message);
...
// Run import
importRelevantProfenderProducts();