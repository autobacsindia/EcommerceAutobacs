...
class EmailHandler {
   ...
   * @param {Array<Object>} options.attachments - Postmark attachments (optional):
  ...
  async sendEmail({ to, subject, text, html, messageStream, attachments, fromEmail, fromName }) {
    ...
    // Resolve sender: allow a per-send override (e.g. hi@ for welcome mail) but only
    // if it's a valid address — otherwise fall back to the configured default sender.
  ...
  /**
   * Send the order confirmation / receipt email with the invoice PDF attached.
   * The order + user are pre-loaded and the invoice PDF is pre-generated by
   * services/invoiceService.js, so this method stays provider-only (no DB access).
   * @param {Object} params
   * @param {string} params.to - Recipient email
   * @param {Object} params.order - Order document (rupee amounts already computed)
   * @param {Object} [params.user] - User document (name)
   * @param {Array<Object>} [params.attachments] - Postmark attachments (invoice PDF)
   * @returns {Promise<Object>} - Result with success status
   */
  async sendOrderConfirmation({ to, order, user = null, attachments }) {
    const { orderConfirmationEmail } = await import('../utils/emailTemplates.js');
    const { companyInfo } = await import('../config/company.js');
    const { subject, text, html } = orderConfirmationEmail({ order, user, company: companyInfo });
   ...
   * The address must be on a verified sender / confirmed domain in Postmark.
   * @param {string} to - Recipient email
   * @param {Object} params
  ...
  async sendWelcomeEmail(to, { name } = {}) {
    const { welcomeEmail } = await import('../utils/emailTemplates.js');
    const { subject, text, html } = welcomeEmail({ name });
    ...
    return this.sendEmail({
      ...
      text,
      html,
      fromEmail: process.env.POSTMARK_WELCOME_FROM_EMAIL || undefined,
      fromName: process.env.POSTMARK_WELCOME_FROM_NAME || undefined,
    });
  }
}
