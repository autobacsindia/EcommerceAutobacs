...
const mockOrderRepo = { findById: jest.fn() };
const mockInvoiceSvc = {
  generateInvoicePdf: jest.fn(),
  invoiceNumber: jest.fn(() => 'AB-34567890'),
};
...
jest.unstable_mockModule('../../../services/invoiceService.js', () => ({
  ...
  invoiceNumber: mockInvoiceSvc.invoiceNumber,
}));
...
describe('orderController.downloadInvoice', () => {
  beforeEach(() => {
    ...
    mockInvoiceSvc.invoiceNumber.mockReturnValue('AB-34567890');
  });
  ...
  test('streams the PDF to the order owner', async () => {
    mockOrderRepo.findById.mockResolvedValue(paidOrder());
    const res = makeRes();
    await downloadInvoice({ params: { id: 'x' }, user: { id: 'user-1', role: 'user' } }, res);
    expect(res.headers['Content-Type']).toBe('application/pdf');
    expect(res.headers['Content-Disposition']).toContain('AB-34567890.pdf');
